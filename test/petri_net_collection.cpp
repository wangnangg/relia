...
void add_trans(PetriNet& builder, TransType type, double val, std::vector<uint_t> in_place_list, std::vector<uint_t> out_place_list)
{
...
PetriNet securityCPS_petri_net()
{
	...
	{
		auto context = static_cast<PetriNetContext*>(context_);
		if (((numG(context) + numB(context) < m) || (numF(context) >= 1) || (numD(context) == 0) || (numG(context) <= 2 * numB(context))) && (numHalt(context) == 0))
		{
			return true;
		}
		else
