...
public:
	...
	void accu_tau(double val) { tau_sum += val; }
	double compute_tau() const { return tau_sum / get_out_sum(); }
};
...
MarkingChain<AcyclicChainElement> generate_next_level_chain(const PetriNet& petri_net, const MarkingChain<AcyclicChainElement>& current_level)
{
...
double compute_acyclic_mtta(const PetriNet& petri_net)
{
	...
	{
		auto van_chain = recursive_explore_vanishing_marking(petri_net, chain_list, current_level, petri_net.get_init_marking().clone());
