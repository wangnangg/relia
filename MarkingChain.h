                                                                                     ...
                                                                                     const IterStopCondition van_chain_stop_condition)
{
    ...
    return {std::move(tan_container_chain), std::move(init_state)};
}
...
template<typename ElementType>
ColSparseM markingchain_to_Qmatrix(const MarkingChain<ElementType> &chain)
{
        ...
        if (ele_ptr->get_out_degree() != 0)
        {
            ...
            for (auto arc : ele_ptr->get_to_arc_list())
            {
                if (arc.val != 0.0)
                {
                    ...
                    uint_t dest_mat_ind = dest_ptr->get_index();
...
template<typename ElementType>
double max_out_rate_sum(const MarkingChain<ElementType> &mk_chain)
{
    double max = -1;
    for (uint_t i = 0; i < mk_chain.size(); i++)
    {
        double sum = mk_chain[i]->get_out_sum();
        if (max < sum)
        {
            max = sum;
        }
